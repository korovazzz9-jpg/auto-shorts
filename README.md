...
Плюс третий пилотный поток: **вьетнамский TikTok** (`Sự Thật 60 Giây`) — random-facts поверх залипательного фона, публикуется вручную (см. раздел «TikTok (вьетнамский, satisfying-формат)»).
...
**Хук-фичи серий выровнены с daily:** каждая часть получает `hook_text` (короткая 3-6 слов on-screen фраза, отличная от озвучки — двойной хук) и тег `hook-<template>` (для `analytics_retention`). Раньше `build_video()` для серий вызывался без `hook_text` — на плашке шёл весь длинный заголовок.

**Визуальный loop-хвост ОТКЛЮЧЁН для серий** (`build_video(part=...)` → `visual_loop=False`): раньше `_build_background` безусловно дублировала первый клип в конец для ЛЮБОГО вызова — бессмысленно для серий, у которых в конце **озвученный** CTA («follow for Part 2/3»), а не молчаливый loop, как у daily.

**Навигация между частями:** Part 1 создаёт **плейлист серии** (`playlists.create_playlist`, id в `state.playlist_id`), каждая часть добавляется в него. Закреп-коммент каждой части → ссылка на плейлист; Part 3 дополнительно даёт прямые ссылки на Part 1/2 (`state.partN_video_id`). Решает проблему «зритель увидел Part 3 — как найти Part 1/2 в бесконечной ленте Shorts».
...
**Тумба** — короткая фраза-хук (`thumb_text`, 3-5 слов, с фолбэком на первые 4 слова заголовка), рисуется крупно (Anton) на затемнённой нижней полосе поверх кадра — а не весь длинный заголовок, как раньше.

**Закреп-коммент** (`CFG["longform_comment"]`) — провокационный вопрос + подписка + само-ответ (мини-тред) — раньше `pipeline_longform.py` вообще не постил коммент.
...
1. **Хук** — интрига БЕЗ раскрытия субъекта. Субъект называется только во 2-м предложении. 5 проверенных шаблонов (`HOOK_TEMPLATES`: vague-ability / you-experience / one-category / authority-cant-explain / sounds-fake / other) — модель отдаёт, каким пользовалась, пайплайн тегирует видео `hook-<template>`, `analytics_retention.py` меряет retention по типу хука.
...
**Двойной хук:** отдельно от произносимой первой фразы, модель отдаёт `hook_text` (3-6 слов, other angle, НЕ копия заголовка) — рисуется на хук-плашке. Глаз и ухо получают две разные зацепки в первые 2 секунды. Фолбэк на `title`, если поле пустое.
...
- **US TikTok (английский, satisfying-формат)** — следующий шаг ПОСЛЕ того, как VN-плейбук докажет себя (не открывать параллельно — фокус). US-SIM/IP при регистрации важен: регион аккаунта прилипчив (см. историю VN-shadowban).
...
## TikTok (вьетнамский, satisfying-формат) — пилот, ручной постинг
...
- `fetch_stock_video.SATISFYING_QUERIES` + `fetch_satisfying_clips()` — фон НЕ тематический, залипательный (kinetic sand, soap cutting, fluid art и т.п.), все запросы проверены на Pexels (коммерческая лицензия, без атрибуции)
