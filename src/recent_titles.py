...
MAX_TITLES = 100
# Кэши — ПО-КАНАЛЬНО (2026-07-10, найдено на ревью): раньше имя было без суффикса канала,
# и в prepare-batch.yml (один job/workspace на EN/ES/PT) каналы делили ОДИН файл: выбор
# темы для ES исключал темы, только что выбранные для EN; avoid-список EN-модели засорялся
# испанскими/португальскими заголовками. Суффикс изолирует каналы даже в общем workspace.
_CACHE_FILE = os.path.join(os.path.dirname(__file__), f"titles_cache_{CHANNEL}.json")
_LEGACY_CACHE_FILE = os.path.join(os.path.dirname(__file__), "titles_cache.json")
_CACHE_MAX = 200

# Темы последних видео — чтобы не выпускать два ролика подряд на одну тему.
_TOPICS_FILE = os.path.join(os.path.dirname(__file__), f"topics_cache_{CHANNEL}.json")
_LEGACY_TOPICS_FILE = os.path.join(os.path.dirname(__file__), "topics_cache.json")
_TOPICS_MAX = 20
...
def _load_list_with_migration(path: str, legacy_path: str) -> list:
    """Читает канальный файл; если его ещё нет — одноразовый фолбэк на старое имя без
    суффикса (актуально при первом прогоне после переименования: старые actions/cache-архивы
    содержат legacy-файл; в канальных воркфлоу он и так одноканальный). Запись всегда идёт
    в новый файл — legacy отмирает сам."""
    if os.path.exists(path):
        return _read_json_list(path)
    return _read_json_list(legacy_path)
...
def add_topic_to_cache(topic: str) -> None:
    ...
    topics = _load_list_with_migration(_TOPICS_FILE, _LEGACY_TOPICS_FILE)
...
def get_recent_topics(n: int = 2) -> list[str]:
    ...
    return _load_list_with_migration(_TOPICS_FILE, _LEGACY_TOPICS_FILE)[:n]


def _load_cache() -> list[str]:
    return _load_list_with_migration(_CACHE_FILE, _LEGACY_CACHE_FILE)
