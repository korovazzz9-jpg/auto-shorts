...
Идея: раз в 4 часа (comment-agent.yml, GitHub Actions cron) один процесс делает ДВЕ вещи
по очереди для обоих каналов (EN/ES):
...
только периодический polling) — при кроне раз в 4 часа между нажатием кнопки и реальной
публикацией ответа может пройти до 4 часов. Если это критично — сократить интервал в
comment-agent.yml (квота позволяет, см. ниже; GitHub Actions-минуты — узкое место, см.
README «GitHub Actions-минуты»).

Квота YouTube (доминирующий бесплатный ресурс, 10 000 юнитов/день, общий пул):
- commentThreads.list = 1 юнит — по разу на канал за прогон = 2 юнита/прогон.
- comments.insert (реальная публикация ответа, только при апруве) = 50 юнитов —
  даже 20 ответов/день = 1000 юнитов. Итог: квота НЕ узкое место (см. README, разбор
  квоты 2026-07-03). Узкое место — GitHub Actions-минуты (см. workflow).
...
комментариев канала ЦЕЛИКОМ (не постранично) — если за 4 часа прилетит больше 20 новых
комментариев по всем видео сразу, самые старые из них выпадут из окна и не будут замечены
(на текущих объёмах — единицы комментариев на видео — маловероятно; при росте канала
можно поднять MAX_FETCH или добавить пагинацию)."""
import json
import os
import time
