...
CONFIGS = {
    ...
    "es": {
        ...
        # 2026-09-07: 4 -> 5 слотов, слот 03:17 убран. География (YouTube Analytics, 60 дней):
        # 87% аудитории в Америках — MX 29.5%, US 26.8%, AR 8.0%, CL 7.2%, CO 6.6%, PE 6.0%,
        # Испания лишь 12.8%. Общее окно бодрствования MX+US = 15:00-02:00 UTC, и замер по 304
        # роликам это подтверждает: 13:17 медиана 1066 (лучший), 00:17 — 1019, 20:17 — 992,
        # а 03:17 (23:17 US-East / 21:17 MX, хвост ночи) — 904, худший. Новые 17:17 и 22:17
        # ложатся в середину окна, разрыв между соседними слотами не меньше 2ч.
        # ⚠️ В cron-job.org: убрать триггер 03:17, добавить 17:17 и 22:17.
        "daily_slots_utc": [(13, 17), (17, 17), (20, 17), (22, 17), (0, 17)],
        ...
        # Обещание расписания (2026-07-18). Цифру не писали, пока слотов было 2 — «2 в день»
        # звучало слабо. 2026-09-07: слотов 5, число само стало аргументом — вернули его в пул
        # (не во все фразы, чтобы при смене расписания не править каждую).
        "cta_schedule_phrases": [
            "5 DATOS NUEVOS AL DÍA\nSUSCRÍBETE",
        ...
        # 2026-09-07: выключал субтитры ради квоты — ОШИБКА, откачено в тот же день.
        # Верные цифры (замер 2026-07-17, сверен с консолью Google, пик 83.56%):
        # лимит 10 000/день, но публикация Shorts стоит ~606 ед., а НЕ 2150 — с 03.07
        # videos.insert вынесен в отдельный бакет и стоит 1 ед. вместо 1600. Разбивка:
        # captions.insert 400 (две трети!) + thumbnails.set 50 + commentThreads.insert 50 +
        # comments.insert 50 + videos.update 50 (localize_metadata) + videos.insert 1 + ~5 чтений.
        # Сходится с фактом: 12 роликов (3 канала) = 7 272 ед., замер тех дней ~7.3k.
        # ES на 5 слотах = ~3 030 из 10 000. Субтитры дороже всего, но запас кратный.
        "captions_enabled": True,
        # 2026-09-07: лонгформы возвращены по решению юзера с переделанной тумбой (жёлтый капс
        # с жирной обводкой, 2-3 слова — см. _save_longform_thumb). Воронка включена обратно.
        # Данные, ради которых стоит перепроверить через месяц: 9 прошлых лонгформов собрали
        # 293 просмотра суммарно, и трафика из ДОМАШНЕЙ ленты у них не было вообще (49.5%
        # «похожие», 37% подписчики) — тумба чинит клик по показу, но не отсутствие показов.
        "longform_funnel": True,
        "first_comment_vote_lines": [
