...
# 2026-07-18: 0.78 → 0.68 — на реальном телефоне (скрин юзера) субтитры на 78% ложились ПОД
# UI Shorts (ник канала/название/«Подписаться» рисуются поверх видео у ВСЕХ зрителей,
# нижние ~22-25% экрана — зона UI). 68% = выше UI, ниже хук-плашки (30%).
CAPTION_Y = int(TARGET_SIZE[1] * 0.68)
...
# 2026-07-18: ротация white/yellow/#7CFFCB остановлена решением юзера (по скрину teal-база
# читается хуже) — база всегда белая, караоке-акцент жёлтый. Тег color-white остаётся в
# аналитике для непрерывности. Вернуть ротацию: добавить цвета обратно в список.
...
def _draw_cta_badge(path: str, text: str, width: int = 760, font_size: int = 62) -> None:
...
def _mid_cta_clip(duration: float) -> list[ImageClip]:
    ...
    2026-07-18 фикс: изначальный размер (width=430, дефолтный font_size=62 как у финального
    CTA) при масштабировании PIL вниз давал мелкий нечитаемый текст, да ещё на 10% высоты —
    почти статус-бар, где взгляд не задерживается. Теперь свой крупный шрифт (52, не «полный»
    68 финального CTA — иначе бейдж стал бы соразмерен basic CTA и терял статус «микро»),
    позиция ниже (16%), плюс лёгкий pulse — иначе статичный бейдж теряется на движущемся фоне."""
    ...
    _draw_cta_badge(badge_png, text, width=520, font_size=52)
    start = duration * MID_CTA_AT
    clip_dur = min(MID_CTA_DURATION, max(duration - start, 0.1))
    pulse = lambda t: 1 + 0.08 * math.sin(t * 2 * math.pi / max(clip_dur, 0.01))
    badge = (
        ImageClip(badge_png)
        .resize(pulse)
        .set_position(("center", int(TARGET_SIZE[1] * 0.16)))
        .set_start(start)
